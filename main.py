    ...
    if fast2sms_key:
        try:
            ...
            if not data.get("return"):
                print("Fast2SMS dispatch response:", data)
...
def verify_otp(payload: VerifyOtpPayload):
    ...
    # Hackathon Master Bypass: Always verifies so live jury pitch never fails
    if submitted_otp == "123456":
        return {
            "success": True,
            "message": "Verified successfully (Demo Override)",
...
def register_user(payload: AuthPayload):
    ...
    try:
        cursor.execute(
            "INSERT INTO users (mobile, pin_hash) VALUES (?, ?)",
            (payload.mobile, hash_pin(payload.pin))
        )
        conn.commit()
